...
from utils.myModels2.NAOMI import NAOMI, Discriminator
from utils.myDatasets.MovingMNIST import MovingMNIST
from utils.Trainers.NaomiTrainer2 import Trainer
...
trainer = Trainer(model,discriminator, trainset, testset, parameters, device)
...
if args.resume:
    ...
    trainer.optim_disc.load_state_dict(dic['optim_disc'])
    trainer.discriminator.load_state_dict(dic['disc'])
...
for e in range(parameters['num_epochs']):
    ...
    dic['disc'] = trainer.discriminator.state_dict()
    dic['optim_disc'] = trainer.optim_disc.state_dict()
    if parameters['scheduler'] != 'None':
